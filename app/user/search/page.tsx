      ...
      <div className="fixed inset-0 md:inset-auto md:relative md:w-full h-screen md:h-auto md:min-h-[70vh] flex flex-col items-center justify-center p-6 text-center">
        <div className="w-full max-w-lg space-y-8">
          <div className="space-y-2">
            ...
            <p className="text-zinc-400 dark:text-zinc-500 font-medium tracking-widest text-xs md:text-sm uppercase">Enter GitHub ID to generate report</p>
          </div>
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto pt-20 px-4">
        ...
        <div className="mt-8 flex justify-center"><SearchInput /></div>
      </div>
    );
  }
      ...
      {userData && (
        <div className="bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] overflow-hidden">
              ...
              <div className="flex-1 space-y-8 w-full text-center lg:text-left">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4">
                    <h3 className="text-4xl sm:text-6xl font-black tracking-[calc(-0.05em)] text-zinc-900 dark:text-white uppercase leading-none italic">
                ...
                <div className="flex divide-x divide-zinc-200 dark:divide-zinc-800 justify-center lg:justify-start border-t border-zinc-100 dark:border-zinc-800/50 pt-8">
                  {[
                    ...
                    <div key={stat.label} className="px-6 sm:px-10 first:pl-0 last:pr-0">
                      ...
                      <span className="text-3xl sm:text-4xl font-black tabular-nums tracking-tighter text-zinc-900 dark:text-zinc-100">
                        {stat.value}
                      </span>
                    </div>
                  ))}
                  ...
                  <div className="relative">
                    <p className="text-zinc-700 dark:text-zinc-200 text-2xl sm:text-4xl font-bold leading-[1.1] tracking-tighter break-keep">
                      {userData.bio ? (
                        ...
                        <span className="text-zinc-300 dark:text-zinc-800">THIS DEVELOPER PREFERS ACTION OVER WORDS.</span>
                      )}
                    </p>
                  </div>
                </div>
          ...
          <div className="p-8 sm:p-12 lg:p-20 space-y-24 bg-zinc-50/50 dark:bg-white/[0.02] border-t border-zinc-100 dark:border-zinc-900">
            ...
            <section className="space-y-10">
              <div className="flex items-center gap-4">
                <span className="w-8 h-8 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 flex items-center justify-center text-[10px] font-black italic">03</span>
                <span className="text-zinc-900 dark:text-zinc-100 font-black text-sm tracking-[0.2em] uppercase">Metadata</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1">
                {[
                  { icon: "📍", label: "Location", value: userData.location || "Earth" },
                  { icon: "🔗", label: "Network", value: userData.blog, isLink: true },
                  { icon: "📧", label: "Contact", value: userData.email || "Private" },
                  { icon: "📅", label: "Member Since", value: new Date(userData.created_at).getFullYear() }
                ].map((item) => (
                  <div key={item.label} className="p-8 bg-white dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 flex flex-col gap-4 first:rounded-l-3xl last:rounded-r-3xl max-lg:rounded-3xl">
                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{item.label}</span>
                    {item.isLink && item.value ? (
                      <a href={item.value.startsWith('http') ? item.value : `https://${item.value}`} target="_blank" className="text-lg font-bold truncate block text-[#5D5FEF] underline underline-offset-4">Visit Site</a>
                    ) : (
                      <p className="text-lg font-bold truncate text-zinc-900 dark:text-zinc-100">{item.value}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="p-12 text-center bg-white dark:bg-[#09090b]">
            <div className="inline-flex items-center gap-4 px-6 py-2 rounded-full border border-zinc-100 dark:border-zinc-800">
              <div className="w-2 h-2 rounded-full bg-green-500"></div>
              <p className="text-[10px] font-black text-zinc-500 tracking-[0.3em] uppercase">
                Analysis Protocol V.2.6 Finalized
              </p>
            </div>
          </div>
        </div>
